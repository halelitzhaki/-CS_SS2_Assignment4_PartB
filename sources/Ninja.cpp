...
void Ninja::move(const Character* enemy) {
    this->setLocation((this->getLocation()).moveTowards(this->getLocation(), enemy->getLocation(), _speed));
}
...
void Ninja::slash(Character* enemy) {
    ...
    if(!isAlive()) throw runtime_error("I'm dead!!");
    if(isAlive() && this->getLocation().distance(enemy->getLocation()) < 1) enemy->hit(40);
}
